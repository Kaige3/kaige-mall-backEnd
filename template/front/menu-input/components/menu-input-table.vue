...
<script lang="ts" setup>
    ...
    import type { MenuInputInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type MenuInputScope = Scope<MenuInputDto['MenuInputRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(menuInputStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...menuInputStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.menuInputController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="菜单名称" prop="getName" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    {{row.getName}}
        </template>
    </el-table-column>
    <el-table-column label="父级菜单" prop="getParentId" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    {{row.getParentId}}
        </template>
    </el-table-column>
    <el-table-column label="菜单路径" prop="getPath" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    {{row.getPath}}
        </template>
    </el-table-column>
    <el-table-column label="排序" prop="getOrderNum" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    {{row.getOrderNum}}
        </template>
    </el-table-column>
    <el-table-column label="菜单类型" prop="getMenuType" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    <dict-column :dict-id="DictConstants.MENU_TYPE" :value="row.getMenuType"></dict-column>
        </template>
    </el-table-column>
    <el-table-column label="图标" prop="getIcon" sortable="custom">
        <template v-slot:default="{row}:MenuInputScope">
                    <el-avatar :src="row.getIcon" alt=""></el-avatar>
        </template>
    </el-table-column>
