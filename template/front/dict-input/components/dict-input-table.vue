...
<script lang="ts" setup>
    ...
    import type { DictInputInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type DictInputScope = Scope<DictInputDto['DictInputRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(dictInputStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...dictInputStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.dictInputController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="值编号" prop="getKeyId" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getKeyId}}
        </template>
    </el-table-column>
    <el-table-column label="值名称" prop="getKeyName" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getKeyName}}
        </template>
    </el-table-column>
    <el-table-column label="值英文名称" prop="getKeyEnName" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getKeyEnName}}
        </template>
    </el-table-column>
    <el-table-column label="字典编号" prop="getDictId" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getDictId}}
        </template>
    </el-table-column>
    <el-table-column label="字典名称" prop="getDictName" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getDictName}}
        </template>
    </el-table-column>
    <el-table-column label="字段英文名称" prop="getDictEnName" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getDictEnName}}
        </template>
    </el-table-column>
    <el-table-column label="排序号" prop="getOrderNum" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.getOrderNum}}
        </template>
    </el-table-column>
    <el-table-column label="是否可见" prop="isVisible" sortable="custom">
        <template v-slot:default="{row}:DictInputScope">
                    {{row.isVisible}}
        </template>
    </el-table-column>
