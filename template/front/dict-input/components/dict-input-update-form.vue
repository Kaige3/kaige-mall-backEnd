<script lang="ts" setup>
    ...
    const {closeDialog, reloadTableData} = dictInputStore
    const {updateForm, dialogData} = storeToRefs(dictInputStore)
    ...
    const rules = reactive<FormRules<typeof updateForm>>({
        ...
        "isVisible": [{ required: true, message: '请输入是否可见', trigger: "blur"}],
    })
    const init = async () => {
        dialogData.value.title = '编辑'
        updateForm.value = {
            ...await api.dictInputController.findById({id: updateForm.value.id || ''})
        }
    }
    watch(
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.dictInputController.save({body: updateForm.value}).then(async (res) => {
...
<template>
    ...
    <el-form-item label="是否可见" prop="isVisible">
                <el-input-number v-model="updateForm.isVisible"></el-input-number>
    </el-form-item>
        </el-form>
