...
<script lang="ts" setup>
    ...
    type MysteryBoxCategoryScope = Scope<MysteryBoxCategoryDto['MysteryBoxCategoryRepository/COMPLEX_FETCHER']>
    ...
    } = mysteryBoxCategoryStore
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(mysteryBoxCategoryStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...mysteryBoxCategoryStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.mysteryBoxCategoryController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="类别名称" prop="name" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryScope">
                    {{row.name}}
        </template>
    </el-table-column>
    <el-table-column label="类别图标" prop="icon" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryScope">
