<script lang="ts" setup>
    ...
    const {updateForm, dialogData} = storeToRefs(mysteryBoxCategoryInputStore)
    ...
    const init = async () => {
        ...
        updateForm.value = {
            ...await api.mysteryBoxCategoryInputController.findById({id: updateForm.value.id || ''})
        }
    }
    watch(
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.mysteryBoxCategoryInputController.save({body: updateForm.value}).then(async (res) => {
