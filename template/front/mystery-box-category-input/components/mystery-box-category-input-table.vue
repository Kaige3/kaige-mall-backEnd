...
<script lang="ts" setup>
    ...
    import type { MysteryBoxCategoryInputInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type MysteryBoxCategoryInputScope = Scope<MysteryBoxCategoryInputDto['MysteryBoxCategoryInputRepository/COMPLEX_FETCHER']>
    ...
    } = mysteryBoxCategoryInputStore
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(mysteryBoxCategoryInputStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...mysteryBoxCategoryInputStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.mysteryBoxCategoryInputController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="类别名称" prop="getName" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryInputScope">
                    {{row.getName}}
        </template>
    </el-table-column>
    <el-table-column label="类别图标" prop="getIcon" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryInputScope">
                    <el-avatar :src="row.getIcon" alt=""></el-avatar>
        </template>
    </el-table-column>
    <el-table-column label="描述" prop="getDescription" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryInputScope">
                    {{row.getDescription}}
        </template>
    </el-table-column>
    <el-table-column label="排序" prop="getSortOrder" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategoryInputScope">
                    {{row.getSortOrder}}
        </template>
    </el-table-column>
