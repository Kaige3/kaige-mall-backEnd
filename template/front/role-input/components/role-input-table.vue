...
<script lang="ts" setup>
    ...
    import type { RoleInputInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type RoleInputScope = Scope<RoleInputDto['RoleInputRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(roleInputStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...roleInputStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.roleInputController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="角色名称" prop="getName" sortable="custom">
        <template v-slot:default="{row}:RoleInputScope">
                    {{row.getName}}
        </template>
    </el-table-column>
