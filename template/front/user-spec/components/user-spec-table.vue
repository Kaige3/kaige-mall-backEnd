...
<script lang="ts" setup>
    ...
    import type { UserSpecInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type UserSpecScope = Scope<UserSpecDto['UserSpecRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(userSpecStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...userSpecStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.userSpecController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="手机号" prop="getPhone" sortable="custom">
        <template v-slot:default="{row}:UserSpecScope">
                    {{row.getPhone}}
        </template>
    </el-table-column>
