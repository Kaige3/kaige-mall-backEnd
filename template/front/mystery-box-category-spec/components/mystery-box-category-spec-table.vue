...
<script lang="ts" setup>
    ...
    import type { MysteryBoxCategorySpecInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type MysteryBoxCategorySpecScope = Scope<MysteryBoxCategorySpecDto['MysteryBoxCategorySpecRepository/COMPLEX_FETCHER']>
    ...
    } = mysteryBoxCategorySpecStore
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(mysteryBoxCategorySpecStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...mysteryBoxCategorySpecStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.mysteryBoxCategorySpecController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="类别名称" prop="getName" sortable="custom">
        <template v-slot:default="{row}:MysteryBoxCategorySpecScope">
                    {{row.getName}}
        </template>
    </el-table-column>
