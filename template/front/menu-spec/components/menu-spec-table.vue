...
<script lang="ts" setup>
    ...
    import type { MenuSpecInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type MenuSpecScope = Scope<MenuSpecDto['MenuSpecRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(menuSpecStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...menuSpecStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.menuSpecController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="菜单名称" prop="getName" sortable="custom">
        <template v-slot:default="{row}:MenuSpecScope">
                    {{row.getName}}
        </template>
    </el-table-column>
