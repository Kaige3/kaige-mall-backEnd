<script lang="ts" setup>
    ...
    const {closeDialog, reloadTableData} = menuSpecStore
    const {updateForm, dialogData} = storeToRefs(menuSpecStore)
    ...
    const init = async () => {
        ...
        updateForm.value = {
            ...await api.menuSpecController.findById({id: updateForm.value.id || ''})
        }
    }
    watch(
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.menuSpecController.save({body: updateForm.value}).then(async (res) => {
