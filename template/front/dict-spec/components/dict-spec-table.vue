...
<script lang="ts" setup>
    ...
    import type { DictSpecInput } from '@/apis/__generated/model/static'
    import { Delete, Edit, Plus } from '@element-plus/icons-vue'
    import DictColumn from '@/components/dict/dict-column.vue'
    import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
    type DictSpecScope = Scope<DictSpecDto['DictSpecRepository/COMPLEX_FETCHER']>
    ...
    const {pageData, loading, queryRequest, table, updateForm, createForm} = storeToRefs(dictSpecStore)
    onMounted(() => {
        reloadTableData()
    })
    ...
    const handleCreate = () => {
        openDialog('CREATE')
        createForm.value = {...dictSpecStore.initForm}
    }
    ...
    const handleDelete = (ids: string[]) => {
        ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
            ...
            api.dictSpecController.delete({body: ids}).then((res) => {
...
<template>
    ...
    <el-table-column label="值编号" prop="getKeyId" sortable="custom">
        <template v-slot:default="{row}:DictSpecScope">
                    {{row.getKeyId}}
        </template>
    </el-table-column>
    ...
    <el-table-column label="是否可见" prop="getVisible" sortable="custom">
        <template v-slot:default="{row}:DictSpecScope">
                    {{row.getVisible}}
        </template>
    </el-table-column>
