<script lang="ts" setup>
    ...
    const {updateForm, dialogData} = storeToRefs(userInputStore)
    ...
    const init = async () => {
        ...
        updateForm.value = {
            ...await api.userInputController.findById({id: updateForm.value.id || ''})
        }
    }
    watch(
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.userInputController.save({body: updateForm.value}).then(async (res) => {
...
<template>
    ...
    <el-form-item label="性别" prop="getGender">
                <dict-select :dict-id="DictConstants.SEX" v-model="updateForm.getGender"></dict-select>
    </el-form-item>
        </el-form>
