<script lang="ts" setup>
  ...
  const res = await api.userForFrontController.findById({ id: updateForm.value.id || '' })
  updateForm.value = { ...res, roleIds: res.rolesView.map((role) => role.id) }
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.userForFrontController.save({body: updateForm.value}).then(async (res) => {
...
<template>
    ...
    <el-form-item label="性别" prop="gender">
                <dict-select :dict-id="DictConstants.SEX" v-model="updateForm.gender"></dict-select>
    </el-form-item>
