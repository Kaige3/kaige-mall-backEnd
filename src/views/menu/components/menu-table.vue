...
<script lang="ts" setup>
    ...
    import type { MenuInput } from '@/apis/__generated/model/static'
...
<template>
    ...
    <el-table-column label="父级菜单" prop="parentId" sortable="custom">
        <template v-slot:default="{row}:MenuScope">
                    {{row.parentId}}
        </template>
    </el-table-column>
    <el-table-column label="菜单路径" prop="path" sortable="custom">
        <template v-slot:default="{row}:MenuScope">
                    {{row.path}}
        </template>
    </el-table-column>
